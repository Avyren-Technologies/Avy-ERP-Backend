...
const REPORT_GENERATOR_MAP: Record<string, (tenantDb: any, companyName: string, filters: DashboardFilters, scope: DataScope, generatedBy?: string, format?: string) => Promise<Buffer>> = {
  ...
  'pip-daily-production': generatePipDailyProductionReport,
  'pip-incentive-summary': generatePipIncentiveSummaryReport,
  'pip-operator-performance': generatePipOperatorPerformanceReport,
  'pip-machine-utilization': generatePipMachineUtilizationReport,
...
class AnalyticsController {
  ...
  getReportCatalog = asyncHandler(async (req: Request, res: Response) => {
    const CATEGORY_META: Record<string, { icon: string; color: string; label: string }> = {
      ...
      Production: { icon: 'factory', color: '#F97316', label: 'Production Reports' },
    };
